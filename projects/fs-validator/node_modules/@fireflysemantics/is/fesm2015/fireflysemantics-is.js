...
class IsError extends Error {
    constructor(message, value, field, constraint, type, code) {
        super(message);
...
function isBooleanError(value, field, code) {
    if (!isBoolean(value)) {
        const message = `The ${value} valued field ${field} should be a boolean.`;
...
function isByteLength(value, min, max) {
    return typeof value === "string" && isByteLength$1(value, { min, max });
}
 ...
 */
function isNumber(target, options = IS_NUMBER_OPTIONS_DEFAULTS) {
...
const IsMobilePhoneLocales = isMobilePhoneLocales;
